...
type ErrorResponse =
  | z.infer<(typeof getFollowingSchema.response)[400]>
  | z.infer<(typeof getFollowingSchema.response)[500]>;

type NotFoundResponse = z.infer<(typeof getFollowingSchema.response)[404]>;
      ...
      .innerJoin(
        (eb) =>
          eb
            ...
            .as('users'),
        (join) =>
          join.onRef('user_relationships.target_id', '=', 'users.user_id')
      )
      .selectAll('users')
      .where('user_relationships.initiator_id', '=', userId)
